...
def apply_envelope(wave, attack, decay, sustain, release):
    ...
    Args:
        ...
        attack (float): Duration of the attack phase.
        decay (float): Duration of the decay phase.
        sustain (float): Sustain level.
        release (float): Duration of the release phase.
...
def generate_sound(frequency, duration, volume=0.5, waveform='sine', attack=0.01, decay=0.1, sustain=0.7, release=0.1):
    ...
    Args:
        frequency (float): Frequency of the sound.
...
explosion_sound = generate_sound(
    frequency=100,
    duration=0.5,
    volume=0.4,
    waveform='noise',
...
life_loss_sound = generate_sound(
    frequency=400,
    duration=0.5,
    volume=0.3,
    waveform='sine',
...
def game_over(screen, score, initials):
    """Display the game over screen and save score.
    
    Args:
        screen: The screen where the game over message is displayed.
    ...
    while game_over_active:
        ...
        for i, (name, points) in enumerate(ranking[:5]):
            color = WHITE if name != initials else YELLOW  # Highlight for current player
...
def capture_initials():
    """Capture player's initials.
    
    Returns:
        str: The player's initials.
    """
    initials = ""
    capturing = True
    while capturing:
...
# Capture player's initials before starting the game
player_initials = capture_initials()
...
while playing:
    ...
    starry_background = StarryBackground(50)

    score = 0
    level = 1
    level_counter = 0
    next_powerup_time = random.randint(500, 1000)
    ...
    while running:
        ...
        all_sprites.update()
        starry_background.update()
        level_counter += 1

        if level_counter >= 1000:
            level_counter = 0
            level += 1
            spawn_frequency = max(500 - level * 50, 100)
            enemy_speed = 2 + level * 0.5

        if random.randint(1, max(60 - level * 2, 10)) == 1:
            enemy_type = random.choice(['normal', 'shooter']) if level >= 3 else 'normal'
            if enemy_type == 'normal':
                enemy = Enemy(level)
            else:
                enemy = ShooterEnemy(level)
        ...
        collision = pygame.sprite.groupcollide(enemies, projectiles, True, True)
        if collision:
        ...
        player_collision = pygame.sprite.spritecollide(player, enemies, True)
        if player_collision:
            if player.shield > 0:
                player.shield -= 1
            else:
        ...
        powerup_collision = pygame.sprite.spritecollide(player, powerups, True)
        ...
        if player.weapon_level > 1:
            weapon_text = game_font.render(f"Weapon Lv{player.weapon_level}", True, WHITE)
            weapon_time = game_font.render(f"Time: {player.powerup_timer // 60}s", True, WHITE)
            SCREEN.blit(weapon_text, (x_offset, 10))
            SCREEN.blit(weapon_time, (x_offset, 30))
        if player.shield > 0:
            shield_text = game_font.render(f"Shield: {player.shield}", True, WHITE)
    ...
    game_over(SCREEN, score, player_initials)
